...
constexpr const char *kDisplayedPeerAddress = "172.26.140.55";
...
void update_node_snapshot(const rk3506_tcp::SensorDataSummary &summary)
{
...
void update_node_info_snapshot(const rk3506_tcp::NodeInfoSummary &summary)
{
    std::lock_guard<std::mutex> lock(state_mutex);
    touch_l2(summary.l2_id);
                     ...
                     const std::string &log_path)
{
    ...
    const char *display_address = kDisplayedPeerAddress;
    ...
    if (!rk3506_tcp::decode_header(
        ...
        {
            std::cerr << "bad UDP frame from " << display_address << ':'
    ...
    if (datagram_len > rk3506_tcp::kMaxDatagramLength)
        {
            std::cerr << "oversized UDP frame from " << display_address << ':'
    ...
    if (expected_len != datagram_len)
        {
            std::cerr << "bad UDP frame length from " << display_address << ':'
    ...
    if (!rk3506_tcp::validate_frame_payload(
        ...
        {
            std::cerr << "bad UDP payload from " << display_address << ':'
                      << ntohs(peer.sin_port) << ": " << error << '\n';
            return;
        }

    std::ostringstream frame_message;
    frame_message << "udp from " << display_address << ':' << ntohs(peer.sin_port)
    ...
    if (header.msg_type
             ...
             == static_cast<std::uint16_t>(rk3506_tcp::MessageType::NodeInfo))
        {
            ...
            if (rk3506_tcp::parse_node_info_binary(
                    ...
                    &error))
                {
                    update_node_info_snapshot(summary);
                    append_log(log_path,
                               "  NODE_INFO l2="
